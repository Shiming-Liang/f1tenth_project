...
class MPC(Node):
    ...
    def __init__(self):
        ...
        self.oa = None
        self.init_flag = 0
        self.track_waypoints = self.waypoints
        self.offset = 0.3
        ...
        self.epsilon = 0.1
        self.k = 1.0
        self.follow_thershold = 0.55


        # initialize MPC problem
        self.mpc_prob_init()

    def opp_odm_sub_callback(self, odom_msg:Odometry):
        ...
        opp_speed = odom_msg.twist.twist.linear.x
        ...
        self.opp_odom = np.array([opp_pos[0], opp_pos[1], opp_yaw,opp_speed])
        # square_waypoints = (self.waypoints[:,:2] - opp_odom[:2])**2
        # min_waypoint = np.argmin(square_waypoints[:,0] - square_waypoints[:,1])
        # #print("***********",np.argmin(square_waypoints[:,0] - square_waypoints[:,1]))
        # self.track_waypoints = self.waypoints[:min_waypoint,:]
        # opp_odom = np.hstack((opp_odom,self.waypoints[min_waypoint,-1]))
        # self.track_waypoints = np.vstack((self.track_waypoints,opp_odom))
    ...
    def pose_callback(self, pose_msg:Odometry):
        ...
        ego_ori = R.from_quat([ori.x, ori.y, ori.z, ori.w])
        ego_yaw = ego_ori.as_euler('zyx')[0]

        vehicle_state.yaw = np.arctan2(2.0 * (ori.w * ori.z + ori.x * ori.y), 1 - 2.0 * (ori.y**2 + ori.z**2))
        ego_pos = np.array([vehicle_state.x, vehicle_state.y]) + self.L * np.array([np.cos(ego_yaw), np.sin(ego_yaw)])

        # update ego odometry
        self.ego_odom = np.array([ego_pos[0], ego_pos[1], ego_yaw,vehicle_state.v])
        ...
        if self.opp_odom is not None:
            #print(np.linalg.norm(self.opp_odom[:2] - self.ego_odom[:2]))
            if np.linalg.norm(self.opp_odom[:2] - self.ego_odom[:2]) < self.follow_thershold:
                msg = AckermannDriveStamped()
                msg.drive.steering_angle = steer_output
                msg.drive.speed = self.opp_odom[-1]
    ...
    def calc_ref_trajectory(self, state, cx, cy, cyaw, sp):
        ...
        if self.opp_odom is not None:
            dis = np.linalg.norm(ref_traj[:2,:].T - self.opp_odom[:2],axis=1)
            #self.opp_odom = np.hstack((self.opp_odom,))
            if np.min(dis) < self.near_threshold:
                ref_traj[:,-2] =  self.opp_odom
                # v1 = np.copy(self.opp_odom)
                next_ref = np.array([self.opp_odom[0], self.opp_odom[1]]) +  self.offset* np.array([np.cos(self.opp_odom[2]), np.sin(self.opp_odom[2])])
                self.publish_marker(next_ref[0],next_ref[1])
                ref_traj[:,-1] = np.array([next_ref[0],next_ref[1],self.opp_odom[2],self.opp_odom[3]])
